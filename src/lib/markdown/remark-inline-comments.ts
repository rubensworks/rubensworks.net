 ...
 * treats it as a block and splits the paragraph in two. One post writes its intro with
 * structural comments between the sentences, which has to stay a single `<p>` — `_base.scss`
 * gives every paragraph a bottom margin, so splitting it changes the spacing.
 *
 * Runs of paragraphs and comment-only HTML nodes with no blank line between them are merged
 * back into one paragraph, with the comment kept as inline HTML.
 */
  ...
  return (tree: Root, file: { value: string }) => {
    ...
    const walk = (parent: Parent) => {
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        // A run must *start* with a paragraph: a comment after a blank line is its own
        // block, and only one between a paragraph's lines counts as inline.
        ...
        if (end === i) continue
        // A run with no comment in it is a lazy continuation CommonMark already folded.
